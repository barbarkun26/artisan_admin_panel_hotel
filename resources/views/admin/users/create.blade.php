...
<div class="max-w-2xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Register New User</h2>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Create a new system user account and assign their access role.</p>
    </div>
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" id="name" required placeholder="User full name" value="{{ old('name') }}"
            ...
            <div>
                ...
                <input type="email" name="email" id="email" required placeholder="user@artisan.com" value="{{ old('email') }}"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:border-indigo-500 text-slate-800 dark:text-slate-100">
            </div>

            <div>
                <label for="phone" class="block text-xs text-slate-400 mb-1 uppercase tracking-wider">Phone Number</label>
                <input type="text" name="phone" id="phone" placeholder="e.g. 0812..." value="{{ old('phone') }}"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:border-indigo-500 text-slate-800 dark:text-slate-100">
            </div>
